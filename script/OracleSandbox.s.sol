...
contract OracleSandboxScript is Script {
  ...
  function run() external {
    ...
    defaultCurrencyName = vm.envOr(
      "DEFAULT_CURRENCY_NAME",
      string("Default Bond Token")
    );
    defaultCurrencySymbol = vm.envOr("DEFAULT_CURRENCY_SYMBOL", string("DBT"));
    defaultCurrencyDecimals = uint8(
      vm.envOr("DEFAULT_CURRENCY_DECIMALS", uint8(18))
    );
    ...
    console.log(
      "Deployed IdentifierWhitelist at %s",
    ...
    OptimisticOracleV3 oo = new OptimisticOracleV3(
      finder,
