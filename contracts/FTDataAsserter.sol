...
contract FTDataAsserter {
  ...
  AucentiveHub public aucentiveHub;
  ...
  constructor(
    ...
    address _aucentiveHub
  ) {
    defaultCurrency = IERC20(_defaultCurrency);
    oo = OptimisticOracleV3Interface(_optimisticOracleV3);
    defaultIdentifier = oo.defaultIdentifier();

    aucentiveHub = AucentiveHub(_aucentiveHub);
  ...
  function assertDataFor(
    ...
    require(aucentiveHub.services(dataId).createdAt != 0, "Service not found");
    ...
    require(data == bytes32("yes") || data == bytes32("no"), "Invalid data");
  ...
  function setAucentiveHub(address _aucentiveHub) public onlyOwner {
    aucentiveHub = AucentiveHub(_aucentiveHub);
  }

  function setDefaultCurrency(address _defaultCurrency) public onlyOwner {
    defaultCurrency = IERC20(_defaultCurrency);
  }
}
