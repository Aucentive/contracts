...
import "./OptimisticOracleV3Fixture.sol";

import "forge-std/Test.sol";

contract CommonOptimisticOracleV3Test is Test {
  ...
  function _commonSetup() public {
    OptimisticOracleV3Fixture.OptimisticOracleV3Contracts
      memory ooContracts = new OptimisticOracleV3Fixture().setUp();
  ...
  function _assertWithCallbackRecipientAndSs(
    address callbackRecipient,
    address escalationManager
  ...
  function _wrappedAssertWithCallbackRecipientAndSs(
    address callbackRecipient,
    address escalationManager
  ) internal returns (bytes32) {
    vm.prank(TestAddress.account1);
  ...
  function _disputeAndGetOracleRequest(
    bytes32 assertionId,
    uint256 bond
  ) internal returns (OracleRequest memory) {
    ...
    OracleRequest memory oracleRequest = OracleRequest({
  ...
  function _allocateBondAndAssertTruth(
    address asserter,
    bytes memory claim
  ...
  function _expectAssertionDisputedCallback(
    address callbackRecipient,
    bytes32 assertionId
  ) internal {
    vm.expectCall(
  ...
  function _expectOraclePriceRequest(
  ...
  function _mockGetAssertionAssertingCaller(
    address mockAssertingCaller,
    bytes32 assertionId
  ) public {
    ...
    vm.mockCall(
      ...
      abi.encodeWithSelector(
        OptimisticOracleV3Interface.getAssertion.selector,
        assertionId
      ),
  ...
  function _defaultCheckBalancesAfterSettle(
    ...
    uint256 expectedOracleFee = payOracleFee
      ? (defaultBond * burnedBondPercentage) / 1e18
      : 0;
    uint256 expectedBondRecipientAmount = defaultBond *
      multiplier -
      expectedOracleFee;
    uint256 expectedAsserterPayout = resolvedTruethful
      ? expectedBondRecipientAmount
      : 0;
    uint256 expectedDisputerPayout = resolvedTruethful
      ? 0
      : expectedBondRecipientAmount;
    ...
    assertEq(
      ...
      balancesBeforeSettle.store + expectedOracleFee
    );
  }
}
