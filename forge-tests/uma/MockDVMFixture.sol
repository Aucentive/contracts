...
contract MockDvmFixture is Test {
  ...
  function setUp() public returns (BaseMockDvmContracts memory) {
    BaseDvmFixture.BaseDvmContracts
      memory baseDvmContracts = new BaseDvmFixture().setUp();

    MockOracleAncillary mockOracle = new MockOracleAncillary(
      address(baseDvmContracts.finder),
      address(baseDvmContracts.timer)
    );
    vm.prank(TestAddress.owner);
    baseDvmContracts.finder.changeImplementationAddress(
...
contract MockDvmFixtureTest is Test {
  function testMockOracle() public {
    MockDvmFixture.BaseMockDvmContracts
      memory baseDvmMockContracts = new MockDvmFixture().setUp();
