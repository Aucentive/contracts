...
contract OptimisticOracleV3Fixture is Test {
  ...
  function setUp() public returns (OptimisticOracleV3Contracts memory) {
    MockDvmFixture.BaseMockDvmContracts
      memory baseMockDvmContracts = new MockDvmFixture().setUp();

    TestnetERC20 defaultCurrency = new TestnetERC20(
      "Default Bond Token",
      "DBT",
      18
    );
    ...
    baseMockDvmContracts.addressWhitelist.addToWhitelist(
      address(defaultCurrency)
    );
    baseMockDvmContracts.identifierWhitelist.addSupportedIdentifier(
      "ASSERT_TRUTH"
    );
    uint256 defaultCurrencyFinalFee = 50e18; // Half of expected minimum bond.
    uint64 defaultLiveness = 7200; // 2 hours
    baseMockDvmContracts.store.setFinalFee(
      address(defaultCurrency),
      FixedPoint.Unsigned(defaultCurrencyFinalFee)
    );
    OptimisticOracleV3Test optimisticOracleV3 = new OptimisticOracleV3Test(
...
contract OptimisticOracleV3FixtureTest is Test {
  function testDefaultConfiguration() public {
    ...
    ooContracts.addressWhitelist.isOnWhitelist(
      address(ooContracts.defaultCurrency)
    );
    ooContracts.identifierWhitelist.isIdentifierSupported("ASSERT_TRUTH");
    assertEq(
      address(ooContracts.optimisticOracleV3.defaultCurrency()),
      address(ooContracts.defaultCurrency)
    );
    assertEq(
      ooContracts.optimisticOracleV3.getMinimumBond(
        address(ooContracts.defaultCurrency)
      ),
      100e18
    );
